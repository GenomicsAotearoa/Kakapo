...
#   plot-vcfstats -p q20 stats
#
# Edit as necessary and recreate the plots by running
#   python plot.py
#
# Title abbreviations:
# 	 0 .. deepv .. deepvariant.r1.q20.allelicprimitives.sorted.bcf
# 	 1 .. freeb .. freebayes.r1.q20.allelicprimitives.sorted.bcf
# 	 2 .. deepv + freeb .. deepvariant.r1.q20.allelicprimitives.sorted.bcf
#
...
if plot_venn_snps:
	...
	ax1.bar([1,2,3],[1307791,831405,256248],align='center',color='orange',width=0.3)
	ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
	ax1.set_xlim(0.5,3.5)
	plt.xticks([1,2,3],('deepv\nts/tv 2.08\n1,307,791','deepv + freeb\nts/tv 2.24\n831,405','freeb\nts/tv 0.89\n256,248'))
...
if plot_venn_indels:
	fig = plt.figure(figsize=(4.33070866141732,3.93700787401575))
	ax1 = fig.add_subplot(111)
	ax1.bar([1,2,3],[310340,90537,107793],align='center',color='red',width=0.3)
	ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
	ax1.set_xlim(0.5,3.5)
	plt.xticks([1,2,3],('deepv\n310,340','deepv + freeb\n90,537','freeb\n107,793'))
...
if plot_SNP_count_by_af:
	...
	if 0 in dat and len(dat[0])>2:
		ax1.plot([row[0] for row in dat[0]], [row[1] for row in dat[0]], '-o',markersize=3, color='orange',mec='orange',label='deepv')
		has_data = 1

	if 1 in dat and len(dat[1])>2:
		ax1.plot([row[0] for row in dat[1]], [row[1] for row in dat[1]], '-o',markersize=3, color='red',mec='red',label='freeb')
		has_data = 1

	if 2 in dat and len(dat[2])>2:
		ax1.plot([row[0] for row in dat[2]], [row[1] for row in dat[2]], '-o',markersize=3, color='darkgreen',mec='darkgreen',label='deepv + freeb')
		has_data = 1
...
if plot_SNP_overlap_by_af and len(dat)>1:
	...
	ax1.set_ylabel('Fraction found in freeb [%]')
	ax1.set_xscale('log')
	ax1.set_xlabel('Non-reference allele frequency in deepv')
	ax1.set_xlim(0,1.01)
