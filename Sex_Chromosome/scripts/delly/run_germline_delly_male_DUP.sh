...
time delly merge -o ${output}male_DUP_sites.bcf ${samps}
...
for samp in ${workbam}*.sorted.bam
	...
	time delly call -g ${ref} -v ${output}male_DUP_sites.bcf -o ${output}${base}.geno.dup.bcf -t DUP ${samp}
done

echo "Merging all the genotyped duplications into single BCF..."
samps=$(ls -lh ${output}*.geno.dup.bcf | awk '{print $9}' | xargs)
time bcftools merge -m id -O b -o ${output}merged_germline_DUP_male.bcf ${samps}
time bcftools index ${output}merged_germline_DUP_male.bcf

echo "Applying the Delly germline SV filter to the merged duplications..."
time delly filter -f germline -o ${output}merged_germline_DUP_male_filtered.bcf ${output}merged_germline_DUP_male.bcf
