...
time delly merge -o ${output}male_INV_sites.bcf ${samps}
...
for samp in ${workbam}*.sorted.bam
        ...
        time delly call -g ${ref} -v ${output}male_INV_sites.bcf -o ${output}${base}.geno.inv.bcf -t INV ${samp}
done

echo "Merging all the genotyped inversions into single BCF..."
samps=$(ls -lh ${output}*.geno.inv.bcf | awk '{print $9}' | xargs)
time bcftools merge -m id -O b -o ${output}merged_germline_INV_male.bcf ${samps}
time bcftools index ${output}merged_germline_INV_male.bcf

echo "Applying the Delly germline SV filter to the merged inversions..."
time delly filter -f germline -o ${output}merged_germline_INV_male_filtered.bcf ${output}merged_germline_INV_male.bcf
