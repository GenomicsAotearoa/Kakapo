...
time delly merge -o ${output}female_INS_sites.bcf ${samps}
...
for samp in ${workbam}*.sorted.bam
        ...
        time delly call -g ${ref} -v ${output}female_INS_sites.bcf -o ${output}${base}.geno.ins.bcf -t INS ${samp}
done

echo "Merging all the genotyped insertions into a single BCF..."
samps=$(ls -lh ${output}*.geno.ins.bcf | awk '{print $9}' | xargs)
time bcftools merge -m id -O b -o ${output}merged_germline_INS_female.bcf ${samps}
time bcftools index ${output}merged_germline_INS_female.bcf

echo "Applying the Delly germline SV filter to called insertions..."
time delly filter -f germline -o ${output}merged_germline_INS_female_filtered.bcf ${output}merged_germline_INS_female.bcf
