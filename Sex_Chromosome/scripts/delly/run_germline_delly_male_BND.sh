...
time delly merge -o ${output}male_BND_sites.bcf ${samps}
...
for samp in ${workbam}*.sorted.bam
        ...
        time delly call -g ${ref} -v ${output}male_BND_sites.bcf -o ${output}${base}.geno.bnd.bcf -t BND ${samp}
done

echo "Merging all the genotyped translocations into single BCF..."
samps=$(ls -lh ${output}*.geno.bnd.bcf | awk '{print $9}' | xargs)
time bcftools merge -m id -O b -o ${output}merged_germline_BND_male.bcf ${samps}
time bcftools index ${output}merged_germline_BND_male.bcf

echo "Applying the Delly germline SV filter to the merged translocations..."
time delly filter -f germline -o ${output}merged_germline_BND_male_filtered.bcf ${output}merged_germline_BND_male.bcf
