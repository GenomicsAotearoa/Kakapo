...
time delly merge -o ${output}female_DEL_sites.bcf ${samps}
...
for samp in ${workbam}*.sorted.bam
        ...
        time delly call -g ${ref} -v ${output}female_DEL_sites.bcf -o ${output}${base}.geno.del.bcf -t DEL ${samp}
done

echo "Merging all the genotyped deletions into a single BCF..."
samps=$(ls -lh ${output}*.geno.del.bcf | awk '{print $9}' | xargs)
time bcftools merge -m id -O b -o ${output}merged_germline_DEL_female.bcf ${samps}
time bcftools index ${output}merged_germline_DEL_female.bcf

echo "Applying the Delly germline SV filter to the merged deletions..."
time delly filter -f germline -o ${output}merged_germline_DEL_female_filtered.bcf ${output}merged_germline_DEL_female.bcf
